...
function productOpts(selected=''){const list=prodData.filter(p=>p.active!==false);return '<option value="">Select product</option>'+list.map(p=>`<option value="${p.id}" data-part="${esc(p.part_number||'')}" data-name="${esc(p.product_name)}" ${String(p.id)===String(selected)?'selected':''}>${esc(p.product_name)}${p.part_number?' — '+esc(p.part_number):''}</option>`).join('')}
function refreshQuotationProductSelects(){['#qItems .item-product','#invoiceItems .item-product'].forEach(sel=>{$$(sel).forEach(el=>{const current=el.value;el.innerHTML=productOpts(current);el.value=current;const o=el.selectedOptions[0];const row=el.closest('.item-row');if(row)row.querySelector('.item-part').value=o?.dataset.part||'';});});}
function addItem(kind='q',item={}){const body=$(kind==='q'?'#qItems':'#invoiceItems');if(!body)return;const tr=document.createElement('tr');tr.className='item-row';tr.innerHTML=`<td><select class="item-product">${productOpts(item.product_id||'')}</select></td><td><input class="item-part" value="${esc(item.part_number||'')}" readonly></td><td><input class="item-qty" type="number" min="0.01" step="0.01" value="${item.quantity||1}"></td><td><input class="item-price" type="number" min="0" step="0.01" value="${item.unit_price||0}"></td><td><input class="item-discount" type="number" min="0" step="0.01" value="${item.discount||0}"></td><td class="item-total">${fmt(item.line_total||0)}</td><td><button type="button" class="small danger remove-item">×</button></td>`;body.appendChild(tr);const sel=tr.querySelector('.item-product');sel.onchange=()=>{const o=sel.selectedOptions[0];tr.querySelector('.item-part').value=o?.dataset.part||'';calcQuotation()};tr.querySelectorAll('input').forEach(x=>x.oninput=calcQuotation);tr.querySelector('.remove-item').onclick=()=>{tr.remove();calcQuotation()};}
...
function printDocument(type,id){const data=type==='quotation'?quotationData.find(x=>String(x.id)===String(id)):invoiceData.find(x=>String(x.id)===String(id));if(!data)return;const table=type==='quotation'?'quotation_items':'invoice_items';sb.from(table).select('*').eq(type==='quotation'?'quotation_id':'invoice_id',id).then(({data:items,error})=>{if(error)return notice(error.message,true);const c=customerData.find(x=>String(x.id)===String(data.customer_id))||{};const rows=(items||[]).map(x=>`<tr><td>${esc(x.product_name)}</td><td>${esc(x.part_number||'')}</td><td>${x.quantity}</td><td>${fmt(x.unit_price)}</td><td>${fmt(x.line_total)}</td></tr>`).join('');const title=type==='quotation'?'QUOTATION':'INVOICE';const num=type==='quotation'?data.quotation_number:data.invoice_number;const html=`<!doctype html><html><head><meta charset="utf-8"><title>${title} ${num}</title><style>body{font-family:Arial,sans-serif;color:#142b3a;margin:40px}h1{margin:0;color:#0b4f68}h2{font-size:18px}.top{display:flex;justify-content:space-between;border-bottom:2px solid #0b4f68;padding-bottom:18px}.muted{color:#657784;font-size:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:25px;margin:24px 0}.grid div{border:1px solid #dbe5ea;padding:14px;border-radius:8px}table{width:100%;border-collapse:collapse;margin-top:20px}th,td{border-bottom:1px solid #dbe5ea;padding:9px;text-align:left;font-size:12px}th{background:#eef5f8}.tot{margin-left:auto;width:300px;margin-top:18px}.tot div{display:flex;justify-content:space-between;padding:6px;border-bottom:1px solid #eee}.grand{font-size:17px;font-weight:bold}.foot{margin-top:35px;font-size:11px;color:#5f7079}.sig{margin-top:25px;border:1px dashed #9aaeb8;padding:16px;height:70px}.no-print{margin-bottom:20px}@media print{.no-print{display:none!important}}button{padding:8px 12px}</style></head><body><div class="no-print"><button onclick="window.print()">Print / Save as PDF</button></div><div class="top"><div><h1>${esc(COMPANY.name)}</h1><div class="muted">${esc(COMPANY.address)}<br>${esc(COMPANY.phone)} · ${esc(COMPANY.email)}<br>${esc(COMPANY.website)}</div></div><div style="text-align:right"><h2>${title}</h2><b>${esc(num)}</b><div class="muted">Date: ${esc(data.quotation_date||data.invoice_date)}${type==='quotation'?'<br>Valid Until: '+esc(data.valid_until):''}</div></div></div><div class="grid"><div><b>Customer</b><br>${esc(c.company_name||'')}<br>${esc(c.contact_name||'')}<br>${esc(c.address||'')}<br>${esc(c.phone||'')}<br>${esc(c.email||'')}</div><div><b>Terms</b><br>Currency: AED<br>VAT: 5%<br>Payment: ${esc(COMPANY.payment)}<br>Delivery: ${esc(COMPANY.delivery)}</div></div><table><thead><tr><th>Product</th><th>Part No.</th><th>Qty</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table><div class="tot"><div><span>Subtotal</span><b>${fmt(data.subtotal)}</b></div><div><span>Discount</span><b>${fmt(data.discount)}</b></div><div><span>Other Charges</span><b>${fmt(data.other_charges)}</b></div><div><span>VAT (5%)</span><b>${fmt(data.vat_amount)}</b></div><div class="grand"><span>Grand Total</span><b>${fmt(data.grand_total)}</b></div></div><div style="margin-top:25px"><b>Bank Details</b><div class="muted">${esc(COMPANY.bank)} · A/C Name: ${esc(COMPANY.accountName)} · A/C No: ${esc(COMPANY.accountNumber)}<br>IBAN: ${esc(COMPANY.iban)} · SWIFT: ${esc(COMPANY.swift)} · Currency: AED</div></div><div class="sig"><b>Customer Acceptance / Signature</b><br><br>Customer Name: __________________________ &nbsp;&nbsp; Date: __________________</div><div class="foot">This is a Computer Generated Document. No Signature Required.</div></body></html>`;const w=window.open('','_blank');w.document.write(html);w.document.close()})}
$('#qDate').value=new Date().toISOString().slice(0,10);$('#qValid').value=new Date(Date.now()+COMPANY.validity*86400000).toISOString().slice(0,10);
});