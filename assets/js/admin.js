...
async function usageDashboard(){
  ...
  set('#usageProducts', prodData.length.toLocaleString());
  set('#usageCategories', catData.length.toLocaleString());
  set('#usageSubcategories', subcatData.length.toLocaleString());
  set('#usageBrands', brandData.length.toLocaleString());

  const storageLimit=1024*1024*1024;
  let total=0, files=0, failed=false;
  try{
    const {data,error}=await sb.storage.from('product-images').list('',{limit:1000});
    if(error) throw error;
    for(const item of (data||[])){
      if(item.metadata && typeof item.metadata.size==='number'){ total+=item.metadata.size; files++; }
    }
  }catch(e){failed=true;}
  if(failed){
    set('#usageStorage','—');
    set('#usageStorageSub','Could not read bucket usage');
  }else{
    const pct=Math.min(100,total/storageLimit*100);
    set('#usageStorage',formatBytes(total));
    set('#usageStorageSub',`${files.toLocaleString()} file${files===1?'':'s'} · ${pct.toFixed(1)}% of 1 GB`);
    const bar=$('#usageStorageBar'); if(bar)bar.style.width=pct+'%';
  }

  // If the optional database-size RPC exists, show the real database size.
  try{
    const {data,error}=await sb.rpc('get_database_size');
    if(error) throw error;
    const bytes=Number(data||0), limit=500*1024*1024, pct=Math.min(100,bytes/limit*100);
    set('#usageDatabase',formatBytes(bytes));
    set('#usageDatabaseSub',`${pct.toFixed(1)}% of 500 MB`);
    const bar=$('#usageDatabaseBar'); if(bar)bar.style.width=pct+'%';
  }catch(e){
    set('#usageDatabase','N/A');
    set('#usageDatabaseSub','Run optional SQL function to enable');
  }
}
