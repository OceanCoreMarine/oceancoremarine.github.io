...
async function compressImage(file, kind='product') {
  ...
  const maxDim = kind === 'logo' ? 700 : 1280;
...
function edit(id){const p=prodData.find(x=>String(x.id)===String(id)),f=$('#productForm');if(!p)return;f.id.value=p.id;f.product_name.value=p.product_name;f.category_id.value=p.category_id;populateSubcategories(p.category_id,p.subcategory_id||'');f.brand_id.value=p.brand_id||'';f.part_number.value=p.part_number||'';f.oem_number.value=p.oem_number||'';f.model.value=p.model||'';f.short_description.value=p.short_description||'';f.featured.checked=!!p.featured;f.active.checked=!!p.active;$('#ptitle').textContent='Edit Product';$('#cancel').classList.remove('hidden');scrollTo({top:0,behavior:'smooth'})}$('#cancel').onclick=reset;function reset(){const f=$('#productForm');f.reset();f.id.value='';f.active.checked=true;$('#ptitle').textContent='Add Product';$('#cancel').classList.add('hidden')}
...
async function quotes(){const {data,error}=await sb.from('quote_requests').select('*').order('created_at',{ascending:false});if(error)return notice(`Quote requests unavailable: ${error.message}. Run quote-workflow-setup.sql first.`,true);const rows=data||[],states=['New','Contacted','Quoted','Won','Lost'],counts=Object.fromEntries(states.map(x=>[x,rows.filter(q=>(q.status||'New')===x).length]));const pipeline=$('#quotePipeline');if(pipeline)pipeline.innerHTML=states.map(x=>`<div class="pipeline-item"><span>${x}</span><b>${counts[x]}</b></div>`).join('');const selected=$('#quoteStatusFilter')?.value||'all',visible=selected==='all'?rows:rows.filter(q=>(q.status||'New')===selected),table=$('#quoteTable');table.innerHTML=visible.map(q=>`<tr><td>${new Date(q.created_at).toLocaleDateString()}</td><td><b>${esc(q.customer_name)}</b><br>${esc(q.company_name||'')}</td><td><a href="mailto:${encodeURIComponent(q.email||'')}">${esc(q.email)}</a><br>${q.phone?`<a href="tel:${esc(q.phone)}">${esc(q.phone)}</a>`:''}</td><td>${esc(q.product_name||'')}<br>${q.part_number?`<b>${esc(q.part_number)}</b>`:''}</td><td>${esc(q.message||'')}${q.attachment_path?`<br><button class="small" type="button" data-quote-file="${esc(q.attachment_path)}">Open attachment</button>`:''}</td><td><label class="sr-only" for="quote-status-${q.id}">Status for ${esc(q.customer_name)}</label><select id="quote-status-${q.id}" class="quote-status" data-quote-status="${q.id}">${states.map(x=>`<option ${((q.status||'New')===x)?'selected':''}>${x}</option>`).join('')}</select></td><td><label class="sr-only" for="quote-followup-${q.id}">Follow-up date for ${esc(q.customer_name)}</label><input id="quote-followup-${q.id}" class="quote-followup" type="date" value="${esc((q.follow_up_at||'').slice(0,10))}" data-quote-followup="${q.id}"></td><td><button class="small danger" data-delquote="${q.id}">Delete</button></td></tr>`).join('')||'<tr><td colspan="8">No quote requests for this status.</td></tr>';$$('[data-quote-status]').forEach(el=>el.onchange=async()=>{const {error}=await sb.from('quote_requests').update({status:el.value}).eq('id',el.dataset.quoteStatus);if(error){notice(error.message,true);return}notice('Quote status updated');await quotes()});$$('[data-quote-followup]').forEach(el=>el.onchange=async()=>{const {error}=await sb.from('quote_requests').update({follow_up_at:el.value||null}).eq('id',el.dataset.quoteFollowup);if(error)return notice(error.message,true);notice('Follow-up date saved')});$$('[data-quote-file]').forEach(b=>b.onclick=async()=>{const {data,error}=await sb.storage.from('quote-attachments').createSignedUrl(b.dataset.quoteFile,120);if(error)return notice(error.message,true);window.open(data.signedUrl,'_blank','noopener,noreferrer')});$$('[data-delquote]').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this quote request and its attachment permanently? This cannot be undone.'))return;const q=rows.find(x=>String(x.id)===b.dataset.delquote);if(q?.attachment_path){const {error}=await sb.storage.from('quote-attachments').remove([q.attachment_path]);if(error)return notice(error.message,true)}const {error}=await sb.from('quote_requests').delete().eq('id',b.dataset.delquote);if(error)return notice(error.message,true);notice('Quote request deleted');quotes()})}
$('#quoteStatusFilter')?.addEventListener('change',quotes);
