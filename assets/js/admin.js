...
function adminList(key, tableId, data, rowFn, bindFn, searchTextFn){
  const table=$('#'+tableId); if(!table)return;
  let st=adminListState[key]||(adminListState[key]={page:1,search:''});
  const size=20;
  const parent=table.closest('.tablewrap')||table.parentElement;
  let controls=$('#'+tableId+'Controls');
  if(!controls){
    controls=document.createElement('div'); controls.id=tableId+'Controls'; controls.className='admin-list-controls';
    table.parentElement.parentElement.insertBefore(controls,table.parentElement);
  }
  const filtered=(data||[]).filter(x=>!st.search||searchTextFn(x).toLowerCase().includes(st.search.toLowerCase()));
  const pages=Math.max(1,Math.ceil(filtered.length/size));
  if(st.page>pages)st.page=pages;
  const start=(st.page-1)*size;
  table.innerHTML=filtered.slice(start,start+size).map(rowFn).join('');
  controls.innerHTML=`<div class="admin-list-toolbar"><input type="search" id="${tableId}Search" value="${esc(st.search)}" placeholder="Search..." aria-label="Search"><span>${filtered.length} item${filtered.length===1?'':'s'}</span></div><div class="pagination" id="${tableId}Pagination"></div>`;
  const input=$('#'+tableId+'Search'); input.oninput=()=>{st.search=input.value;st.page=1;adminList(key,tableId,data,rowFn,bindFn,searchTextFn)};
  const pg=$('#'+tableId+'Pagination');
  let html=`<button class="small" ${st.page<=1?'disabled':''} data-page="${st.page-1}">Previous</button>`;
  const cur=st.page, nums=[];
  if(pages<=7){for(let i=1;i<=pages;i++)nums.push(i)}else{nums.push(1);let a=Math.max(2,cur-2),b=Math.min(pages-1,cur+2);if(a>2)nums.push('…');for(let i=a;i<=b;i++)nums.push(i);if(b<pages-1)nums.push('…');nums.push(pages)}
  for(const n of nums) html+=n==='…'?'<span>…</span>':`<button class="small ${n===cur?'active':''}" data-page="${n}">${n}</button>`;
  html+=`<button class="small" ${st.page>=pages?'disabled':''} data-page="${st.page+1}">Next</button>`;
  pg.innerHTML=html; pg.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{st.page=Number(b.dataset.page);adminList(key,tableId,data,rowFn,bindFn,searchTextFn)});
  bindFn(table.querySelectorAll('button'));
}
let catData=[],subcatData=[],brandData=[],prodData=[];async function categories(){const {data,error}=await sb.from('categories').select('*').order('name');if(error)return notice(error.message,true);catData=data||[];const opts=catData.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');$('#catSelect').innerHTML=opts;$('#subcatCategory').innerHTML=opts;adminList('categories','categoryTable',catData,c=>`<tr><td>${esc(c.name)}</td><td><button class="small danger" data-delcat="${c.id}">Delete</button></td></tr>`,buttons=>buttons.forEach(b=>b.onclick=async()=>{if(!confirm('Delete this category?'))return;const {error}=await sb.from('categories').delete().eq('id',b.dataset.delcat);if(error)notice(error.message,true);else{notice('Category deleted');categories();products();setTimeout(refreshUsage,150)}}),c=>c.name||'')}
async function subcategories(){const {data,error}=await sb.from('subcategories').select('*,categories(name)').order('name');if(error)return notice(error.message,true);subcatData=data||[];adminList('subcategories','subcategoryTable',subcatData,x=>`<tr><td>${esc(x.categories?.name||'')}</td><td><b>${esc(x.name)}</b></td><td><div class="actions"><button class="small" data-editsub="${x.id}">Edit</button><button class="small danger" data-delsub="${x.id}">Delete</button></div></td></tr>`,buttons=>{buttons.forEach(b=>b.dataset.editsub&&(b.onclick=()=>editSubcategory(b.dataset.editsub)));buttons.forEach(b=>b.dataset.delsub&&(b.onclick=async()=>{if(!confirm('Delete this sub-category?'))return;const {error}=await sb.from('subcategories').delete().eq('id',b.dataset.delsub);if(error)notice(error.message,true);else{notice('Sub-category deleted');subcategories();products()}}))},x=>`${x.name||''} ${x.categories?.name||''}`)}
...
async function products(page=productPage, search=productSearchTerm){
  ...
  prodData=data||[];
  const allRes=await sb.from('products').select('*,categories(name),brands(name)').order('product_name');
  if(!allRes.error) allProductData=allRes.data||[];
...
async function quotes(){const {data,error}=await sb.from('quote_requests').select('*').order('created_at',{ascending:false});if(error)return notice(error.message,true);const rows=data||[];adminList('quotes','quoteTable',rows,q=>`<tr><td>${new Date(q.created_at).toLocaleString()}</td><td><b>${esc(q.customer_name)}</b><br>${esc(q.company_name||'')}</td><td>${esc(q.email)}<br>${esc(q.phone||'')}</td><td>${esc(q.product_name||'')}<br>${esc(q.part_number||'')}</td><td>${esc(q.message||'')}</td><td>${esc(q.status||'New')}</td><td><div class="actions"><button class="small" data-contact="${q.id}">Mark Contacted</button><button class="small danger" data-delquote="${q.id}">Delete</button></div></td></tr>`,buttons=>{buttons.forEach(b=>b.dataset.contact&&(b.onclick=async()=>{const {error}=await sb.from('quote_requests').update({status:'Contacted'}).eq('id',b.dataset.contact);if(error)notice(error.message,true);else{notice('Quote request marked as contacted');quotes()}}));buttons.forEach(b=>b.dataset.delquote&&(b.onclick=async()=>{if(!confirm('Delete this quote request permanently? This cannot be undone.'))return;const {error}=await sb.from('quote_requests').delete().eq('id',b.dataset.delquote);if(error)return notice(error.message,true);notice('Quote request deleted');quotes()}))},q=>`${q.customer_name||''} ${q.company_name||''} ${q.email||''} ${q.phone||''} ${q.product_name||''} ${q.part_number||''} ${q.message||''} ${q.status||''}`)}
...
async function customers(){const {data,error}=await sb.from('customers').select('*').order('created_at',{ascending:false});if(error)return notice(error.message,true);customerData=data||[];const q=$('#qCustomer');if(q)q.innerHTML=customerOpts(q.value);adminList('customers','customerTable',customerData,c=>`<tr><td>${esc(c.company_name||'')}</td><td>${esc(c.contact_name)}</td><td>${esc(c.email||'')}</td><td>${esc(c.phone||'')}</td><td><button class="small" data-editcustomer="${c.id}">Edit</button> <button class="small danger" data-delcustomer="${c.id}">Delete</button></td></tr>`,buttons=>{buttons.forEach(b=>b.dataset.editcustomer&&(b.onclick=()=>editCustomer(b.dataset.editcustomer)));buttons.forEach(b=>b.dataset.delcustomer&&(b.onclick=async()=>{if(!confirm('Delete this customer?'))return;const r=await sb.from('customers').delete().eq('id',b.dataset.delcustomer);if(r.error)notice(r.error.message,true);else{notice('Customer deleted');customers()}}))},c=>`${c.company_name||''} ${c.contact_name||''} ${c.email||''} ${c.phone||''}`)}
...
function productList(){return (allProductData.length?allProductData:prodData).filter(p=>p.active!==false)}
...
async function quotations(){const {data,error}=await sb.from('quotations').select('*,customers(company_name,contact_name)').order('created_at',{ascending:false});if(error)return notice(error.message,true);quotationData=data||[];adminList('quotations','quotationTable',quotationData,q=>`<tr><td><b>${esc(q.quotation_number)}</b></td><td>${esc(q.customers?.company_name||q.customers?.contact_name||'')}</td><td>${esc(q.quotation_date)}</td><td>${fmt(q.grand_total)}</td><td><span class="status">${esc(q.status)}</span></td><td><button class="small" data-editq="${q.id}">Edit</button> <button class="small" data-printq="${q.id}">Print / PDF</button> <button class="small" data-invoiceq="${q.id}">Convert to Invoice</button> <button class="small danger" data-delq="${q.id}">Delete</button></td></tr>`,buttons=>{buttons.forEach(b=>b.dataset.editq&&(b.onclick=()=>startQuotationEdit(b.dataset.editq)));buttons.forEach(b=>b.dataset.printq&&(b.onclick=()=>printDocument('quotation',b.dataset.printq)));buttons.forEach(b=>b.dataset.invoiceq&&(b.onclick=()=>convertInvoice(b.dataset.invoiceq)));buttons.forEach(b=>b.dataset.delq&&(b.onclick=()=>deleteQuotation(b.dataset.delq)))},q=>`${q.quotation_number||''} ${q.customers?.company_name||''} ${q.customers?.contact_name||''} ${q.quotation_date||''} ${q.status||''}`)}
...
async function invoices(){const {data,error}=await sb.from('invoices').select('*,customers(company_name,contact_name)').order('created_at',{ascending:false});if(error)return notice(error.message,true);invoiceData=data||[];adminList('invoices','invoiceTable',invoiceData,i=>`<tr><td><b>${esc(i.invoice_number)}</b></td><td>${esc(i.customers?.company_name||i.customers?.contact_name||'')}</td><td>${esc(i.invoice_date)}</td><td>${fmt(i.grand_total)}</td><td><span class="status">${esc(i.payment_status)}</span></td><td><button class="small" data-printi="${i.id}">Print / PDF</button> <button class="small danger" data-deli="${i.id}">Delete</button></td></tr>`,buttons=>{buttons.forEach(b=>b.dataset.printi&&(b.onclick=()=>printDocument('invoice',b.dataset.printi)));buttons.forEach(b=>b.dataset.deli&&(b.onclick=()=>deleteInvoice(b.dataset.deli)))},i=>`${i.invoice_number||''} ${i.customers?.company_name||''} ${i.customers?.contact_name||''} ${i.invoice_date||''} ${i.payment_status||''}`)}
