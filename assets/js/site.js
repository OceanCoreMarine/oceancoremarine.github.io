...
function card(p){const cat=p.categories?.name||'';const subcat=p.subcategories?.name||'';const brand=p.brands?.name||'';const desc=p.short_description||'Marine spare part or equipment.';return `<article class="product"><div class="visual"><a class="product-card-link" href="product.html?id=${encodeURIComponent(p.id)}">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.product_name)}" loading="lazy">`:esc((cat||'MAR').slice(0,3).toUpperCase())}</a></div><div class="body"><div class="product-meta"><span>${esc(cat||'Marine')}</span>${subcat?`<span>› ${esc(subcat)}</span>`:''}</div><a class="product-card-link" href="product.html?id=${encodeURIComponent(p.id)}"><h3>${esc(p.product_name)}</h3></a>${brand?`<p class="product-detail"><b>Brand:</b> ${esc(brand)}</p>`:''}${p.part_number?`<p class="product-detail"><b>Part No:</b> ${esc(p.part_number)}</p>`:''}<p class="product-desc"><span class="desc-text">${esc(desc)}</span><button type="button" class="see-more">See more</button></p><button class="btn btn-primary quote-product" data-name="${esc(p.product_name)}" data-part="${esc(p.part_number||'')}">Request a Quote</button></div></article>`}
...
const detailRoot=document.querySelector('#productDetails');
if(detailRoot){
  const detailParams=new URLSearchParams(location.search); const productId=detailParams.get('id');
  const quoteToProduct=(p)=>location.href=`index.html?product=${encodeURIComponent(p.product_name)}&part=${encodeURIComponent(p.part_number||'')}#quote`;
  const detailCard=(p)=>card(p);
  if(!productId){detailRoot.innerHTML='<div class="empty">Product not found. <a href="products.html">Return to catalogue</a></div>'}
  else{
    const p=ps.find(x=>String(x.id)===String(productId));
    if(!p){detailRoot.innerHTML='<div class="empty">Product not found. <a href="products.html">Return to catalogue</a></div>'}
    else{
      const cat=p.categories?.name||'Marine'; const sub=p.subcategories?.name||''; const brand=p.brands?.name||'';
      detailRoot.innerHTML=`<a class="back-link" href="javascript:history.back()">← Back</a><div class="product-detail-layout"><div class="product-detail-image">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.product_name)}">`:`<div class="detail-placeholder">${esc((cat||'MAR').slice(0,3).toUpperCase())}</div>`}</div><div class="product-detail-info"><div class="detail-kicker"><span>${esc(cat)}</span>${sub?`<span>${esc(sub)}</span>`:''}</div><h1>${esc(p.product_name)}</h1><div class="detail-meta">${brand?`<div><small>Brand</small><strong>${esc(brand)}</strong></div>`:''}${p.part_number?`<div><small>Part Number</small><strong>${esc(p.part_number)}</strong></div>`:''}<div><small>Category</small><strong>${esc(cat)}</strong></div>${sub?`<div><small>Sub-category</small><strong>${esc(sub)}</strong></div>`:''}</div><div class="detail-description">${esc(p.short_description||'Marine spare part or equipment.')}</div><button class="btn btn-primary" id="detailQuote">Request a Quote</button></div></div>`;
      document.querySelector('#detailQuote')?.addEventListener('click',()=>quoteToProduct(p));
      const similar=ps.filter(x=>String(x.id)!==String(p.id)&&((p.subcategory_id&&String(x.subcategory_id)===String(p.subcategory_id))||(p.category_id&&String(x.category_id)===String(p.category_id))||(p.brand_id&&String(x.brand_id)===String(p.brand_id))));
      // Prioritize same sub-category, then category, then brand; remove duplicates.
      similar.sort((a,b)=>{const score=x=>(p.subcategory_id&&String(x.subcategory_id)===String(p.subcategory_id)?3:0)+(p.category_id&&String(x.category_id)===String(p.category_id)?2:0)+(p.brand_id&&String(x.brand_id)===String(p.brand_id)?1:0);return score(b)-score(a)});
      const sg=document.querySelector('#similarProducts'); if(sg){sg.innerHTML=similar.length?similar.slice(0,8).map(detailCard).join(''):'<div class="empty">No similar products available yet.</div>';bindProductCards(sg)}
    }
  }
}
